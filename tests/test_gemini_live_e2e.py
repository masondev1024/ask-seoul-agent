...
def _live_secret(flag: str) -> str:
    if os.getenv(flag) != "1":
        pytest.skip(f"set {flag}=1 to explicitly authorize Gemini live evaluation")
...
@pytest.mark.live
@pytest.mark.anyio
async def test_gemini_live_eval_uses_recorded_tool_fixtures() -> None:
    """Variable live model evaluation over deterministic, privacy-bounded tool fixtures."""
    from ask_seoul_agent.evaluation import load_eval_manifest
    from ask_seoul_agent.live_eval import run_live_suite
    from ask_seoul_agent.providers.gemini import (
        DEFAULT_MODEL,
...
@pytest.mark.live
def test_gemini_and_ask_seoul_live_sse_e2e() -> None:
    """Real Gemini plus live ASK Seoul REST, traversing the public FastAPI SSE contract."""
    from ask_seoul_agent.api import create_app
    from ask_seoul_agent.providers.gemini import DEFAULT_MODEL
    ...
    assert final["envelope"]["provider"] == "gemini"
